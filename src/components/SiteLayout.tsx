import { CalendarDays, ChevronRight, Clock3, MapPin, Menu, MessageCircle, Phone } from "lucide-react";
...
function LanguageSwitch({ className }: { className?: string }) { const { language, setLanguage } = useI18n(); const labels: [Language, string][] = [["en", "EN"], ["mr", "मराठी"], ["hi", "हिन्दी"]]; return <div aria-label="Language" className={cn("flex items-center gap-1 text-xs font-semibold", className)}>{labels.map(([id, label]) => <button key={id} onClick={() => setLanguage(id)} className={`min-h-8 px-2 transition-colors rounded ${language === id ? "text-gold-700 underline underline-offset-4 font-bold" : "text-muted hover:text-navy-900"}`} lang={id}>{label}</button>)}</div>; }
